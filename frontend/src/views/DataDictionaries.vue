<template>
  <div>
    <el-alert
      title="填写方法：字典名称是任务中引用的变量名，例如 promo_text 对应 {promo_text}。文本字典一行一条。图片字典给通用任务配图；素材图片单独维护建群/活跃消息用的固定图，可在线上传、改名、编辑、删除后在任务规则里直接选用，不需要复制一份组合字典。随机模式每次随机取一项，队列模式按顺序轮换。"
      type="info"
      :closable="false"
      show-icon
      class="mb-3"
...
async function saveMediaDictionary() {
  const files: File[] = []
