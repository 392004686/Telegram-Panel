...
export const panelApi = {
  ...
  saveVideoDictionary: (form: FormData) =>
    api.post<DataDictionary>('/data-dictionaries/video', form, { timeout: 300_000 }).then((r) => r.data),
  ...
  createCustomerLookupBatch: (payload: { name?:string; mode:string; targets:string[]; accountIds:number[]; accountCategoryId?:number|null; targetOrder:string; minDelaySeconds:number; maxDelaySeconds:number }) => api.post<{batchId:number;taskId:number}>('/customer-lookup-batches', payload).then(r=>r.data),
