...
export const panelApi = {
  ...
  syncSelectedGroups: (accountId: number, groupIds: number[]) =>
    api.post<SyncResult>('/groups/sync-selected', { accountId, groupIds }, { timeout: 180_000 }).then((r) => r.data),
  exportGroupLink: (id: number) =>
    api.post<LinkResult>(`/groups/${id}/export-link`, {}, { timeout: 120_000 }).then((r) => r.data),
  exportGroupPrivateInviteLink: (id: number) =>
    api.post<LinkResult>(`/groups/${id}/export-private-link`, {}, { timeout: 120_000 }).then((r) => r.data),
