...
public sealed class CommandConsoleStore
{
    ...
    public IReadOnlyList<RunEvent>? Read(string runId) { var file = Directory.EnumerateFiles(Path.Combine(_root, "runs"), runId + ".jsonl", SearchOption.AllDirectories).SingleOrDefault(); var options = new JsonSerializerOptions(JsonSerializerDefaults.Web); return file is null ? null : File.ReadLines(file).Select(x => JsonSerializer.Deserialize<RunEvent>(x, options)!).ToArray(); }
    public void Clear() { if (Directory.Exists(Path.Combine(_root, "runs"))) Directory.Delete(Path.Combine(_root, "runs"), true); Directory.CreateDirectory(Path.Combine(_root, "runs")); }
    public bool Delete(string runId) { var file = Directory.EnumerateFiles(Path.Combine(_root, "runs"), runId + ".jsonl", SearchOption.AllDirectories).SingleOrDefault(); if (file is null) return false; File.Delete(file); return true; }
}
