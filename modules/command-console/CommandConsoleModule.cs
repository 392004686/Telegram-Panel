...
const ACTION={'group.list':'群组列表','group.get':'群组详情','group.invite':'邀请用户进群','account.sync':'账号同步','chat.join':'加入群组或频道','proxy.list':'代理列表','engagement.group.inspect':'检查可复用群组','engagement.group.available':'查询可用群组','engagement.account.health':'检查执行账号','engagement.batch.preview':'预览客户邀请批次','engagement.batch.run':'执行客户邀请批次'};
