...
{
    public ModuleManifest Manifest => new() { Id = "command-console", Name = "命令控制台", Version = "1.0.11", Host = new HostCompatibility { Min = "1.31.76" }, Entry = new ModuleEntryPoint { Assembly = "TelegramPanel.CommandConsole.dll", Type = GetType().FullName! } };
...
const CATALOG=[['group.list','查询群组列表',[['account','执行账号','',true],['refresh','刷新','true',false]]],['group.get','查询群组详情',[['account','执行账号','',true],['group','群组 ID','',true]]],['group.invite','邀请客户进群',[['account','执行账号','',true],['group','群组 ID','',true],['usernames','客户列表','',true],['delay','间隔毫秒','2000',false]]]];
...
let accountOptions=[],groupOptions=[];
function opts(list,key,label){return '<option value="">请选择</option>'+list.map(x=>'<option value="'+esc(x[key])+'">'+esc(label(x))+'</option>').join('')}
function renderParams(){const a=document.getElementById('action').value,x=CATALOG.find(y=>y[0]===a);document.getElementById('params').innerHTML=(x?.[2]||[]).map(p=>{let control=p[0]==='account'?'<select data-param="account" data-required="'+p[3]+'">'+opts(accountOptions,'id',x=>'#'+x.id+' '+(x.phone||x.username||x.nickname||''))+'</select>':p[0]==='group'?'<select data-param="group" data-required="'+p[3]+'">'+opts(groupOptions,'telegramId',x=>(x.title||'群组')+' · '+x.telegramId)+'</select>':'<input data-param="'+p[0]+'" data-required="'+p[3]+'" value="'+p[2]+'" style="width:140px;padding:8px;border:1px solid #d7dfeb;border-radius:7px">';return '<label>'+p[1]+(p[3]?' *':'')+control+'</label>'}).join('')}
async function loadChoices(){try{const a=await(await fetch('/api/panel/operation-accounts')).json();accountOptions=Array.isArray(a)?a:(a.items||[])}catch{}try{const g=await(await fetch('/api/panel/groups')).json();groupOptions=Array.isArray(g)?g:(g.items||g.data||[])}catch{}renderParams()}
...
internal static class CommandRunner
{
    public static async Task<RunResult> RunAsync(string raw, CommandConsoleStore store, IServiceProvider services, CancellationToken ct)
    {
        ...
        try
        {
            ...
            Add("run.started", new { moduleVersion = "1.0.11" });
