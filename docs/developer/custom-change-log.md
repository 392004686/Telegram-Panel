...
### 2026-09-15 客户页稳定性与账号筛选

- 客户列表删除重复页内标题并统一使用全局 `page-card` 宽度，筛选、批量操作和横向明细表对齐账号列表布局。
- 客户资料扩展用户 ID、姓名、用户名、手机号、头像、活跃状态、Premium、机器人、认证、诈骗、虚假、注销和生日字段；离线状态保留最后上线时间。
- 手机号查询兼容可选 `+`、纯数字及任意空格组合，拒绝其他字符；用户名使用 `@username`。
- “查找用户”拆为客户管理子页“账号筛选”，支持单个/批量、指定账号/账号分类轮询、队列/随机、最小/最大秒间隔及详细结果表。不存在于客户列表的目标只留在本页结果，不自动污染客户数据。
...
### 2026-09-16 客户建群活跃执行器设计

- 新增“批量建群 → 邀请客户 → 执行活跃 → 标记已沟通”正式执行器设计文档。
- 任务建议注册为 `customer_group_engagement`，复用现有 `BatchTasks` 调度、暂停、取消、心跳及启动恢复合同。
- 明确业务持久化表、客户租约、幂等键、每账号独立延迟、错误分类、部分成功和重启恢复策略。
- 默认仅在邀请成功且群内活跃全部完成后标记客户“已沟通”；查询、建群或邀请失败不能伪装为触达成功。
...
### 2026-09-16 截图回归漏洞记录与任务表单增强

- 记录客户列表批量操作栏在窄内容区溢出、垂直未对齐的问题；修复策略为限制操作栏宽度、允许横向滚动并复用固定操作列遮罩。
- 记录指定执行账号缺少取消勾选和当前分类列的问题；查询页已保留“取消全选/移出选择”，并新增当前分类展示。
- 历史查询记录要求继续永久保留、可筛选、可查看明细并支持重试/删除；当前持久化批次接口已覆盖这些操作。
- 批量建群邀请客户并执行活跃已补专用配置表单，包含账号来源、客户分类、数量分配、群模板、活跃文案、并发和延迟参数；提交后进入持久化任务中心。

### 2026-09-17 客户列表操作栏对齐与任务账号错误提示

- 修复客户列表批量操作栏覆盖主内容区的问题；操作栏卡片恢复与筛选区、客户表格相同的最大宽度和水平居中规则。
- 批量建群任务在执行账号分类没有可用账号时，错误信息现在包含具体分类编号及“启用且 Telegram 状态可用”的筛选条件，便于创建任务前排查配置。

### 2026-09-18 建群活跃任务执行确认与日志刷新

- 邀请动作只有在成员快照确认目标 Telegram 用户已入群后才记录为成功，避免 RPC 接受请求但实际未入群时产生假成功。
- 代理连接提前关闭、会话失效和传输连接关闭会将当前执行账号移出本次任务健康池，剩余群组继续使用其他可用账号；日志写明失败原因。
- 群组日志弹窗增加“刷新日志”按钮，无需关闭浮窗即可查看新产生的日志。
- `{seq}` 和 `{date}` 作为建群任务内置变量先展开，不会误读同名文本字典；其他 `{变量}` 仍按字典规则随机取值。

### 2026-09-18 素材状态栏与群组消息规则统一

- 素材图 iOS 时间文字改用与 `购物网站截图素材/mockify-generate.js` 相同的 SVG 基线定位，修复时间相对官方状态栏下移和视觉比例不一致。
- 群组详情移除重复的旧文字/媒体单次发送区，只保留消息规则入口；规则入口增加本地图片/视频、默认勾选合并发送和“立即发送”按钮。
- 频道详情继续保留原单次立即发送表单；群组规则仍支持文字字典、图片字典和素材图。

### 2026-09-18 Telegram 未邀请成员原因解析

- 普通群和超级群邀请现在读取 Telegram `messages.invitedUsers.missing_invitees`，不再把“RPC 未抛异常”直接当作成功。
- 日志区分执行账号需要 Premium、目标邀请隐私限制，以及需要 Premium 才能私聊发送邀请链接；失败结果携带目标用户 ID 和明确原因。
- 成员快照仍作为成功后的二次确认，用于发现 Telegram 返回成功但成员状态异常的情况。
- legacy 群组批量邀请入口同步执行成员快照确认，修复旧入口将未实际入群记录为成功的问题。


## 1.0.6
- 邀请成功判定增加 updates 证据与成员快照短重试，减少任务失败但用户稍后进群的误判。
- 建群活跃任务与 legacy 群组批量邀请统一使用同一套延迟确认逻辑。
- 页面版本号改为 1.0.6。

## 1.01
- 客户列表：活跃状态显示具体时间；查询状态「已找到」改为「已确认」；新增执行状态（未执行/已沟通）；筛选栏取消勾选；批量操作支持勾选本页未执行、当前组已沟通/未执行、标记已沟通。
- 账号筛选历史批次：分页、名称/内容模糊搜索、查询内容列、全选/取消/批量删除、查看预览执行历史。
- 任务中心：新建任务默认置顶「批量建群邀请客户并执行活跃」；活跃消息改为持续活跃同款规则并可插入文字字典；历史任务筛选/全选/批量删除，修复右侧溢出遮挡。
- 顶栏显示自定义版本号 1.01。
...
## 1.0.5
- 素材生成改为严格对齐本地购物网站截图项目：官方状态栏、真实字体时间、3 倍内绘再按自定义倍率缩放，默认 1 倍为 440×956。
- 邀请成功不再把执行账号自己或已在群中的用户计为新邀请；成员数改为读取 Telegram 实际人数。
- 任务日志改为逐步动作：加载账号分类、建群字典随机值、逐人邀请、规则生成/发送计数、标记已沟通。
- 任务增加「强制二次确认」，勾选后可二次执行已沟通客户且不再拦截。
- 客户列表增加批量修改执行状态入口。
- 群组详情增加与任务相同的消息规则添加器，便于单独测试发送。
- 页面版本号改为 1.0.5。
