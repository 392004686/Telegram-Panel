...
public sealed class TaskCatalogModule : ITelegramPanelModule, IModuleTaskProvider
{
    ...
    public IEnumerable<ModuleTaskDefinition> GetTasks(ModuleHostContext context)
    {
        ...
        {
            Category = "user", TaskType = BatchTaskTypes.CustomerLookup, DisplayName = "客户账号筛选",
            Description = "持久化查询客户 Telegram 资料，关闭页面或容器重启后可继续执行。", Icon = Icons.Material.Filled.Search,
            EditorComponentType = typeof(TelegramPanel.Web.Components.Dialogs.GenericJsonTaskEditor).AssemblyQualifiedName ?? "",
            TaskCenter = new ModuleTaskCenterCapabilities { CanPause = true, CanResume = true, CanEdit = false, CanRerun = true }, Order = 101
        };
        yield return new ModuleTaskDefinition
        {
